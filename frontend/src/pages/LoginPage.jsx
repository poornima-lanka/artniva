...
function SellerDashboardPage() {
    ...
    const getImageUrl = (item) => {
        ...
        // Case A: Local Public Folder (Port 3000)
        ...
        if (localImages.includes(fileName)) return `${window.location.origin}/images/${fileName}`;

        // Case B: Dashboard Upload (Starts with /uploads)
        if (cleanPath.includes('uploads/')) {
            const formattedPath = cleanPath.startsWith('/') ? cleanPath : `/${cleanPath}`;
            return `${BACKEND_URL}${formattedPath}`;
        }

        // Case C: Thunder Client (Missing /uploads prefix)
        return `${BACKEND_URL}/uploads/${fileName}`;
    };

    // 2. FETCH DATA: Both Materials and Products fetch chesthundhi
    ...
    // 3. UPLOAD FIX: Sends category and routes correctly
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (activeTab === 'add-material') {
                dataToSend.append('category', 'Material'); // Missing in your old code
